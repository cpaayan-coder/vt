...
def edit_product(product_id):
    ...
    if request.method == "POST":
        ...
        if name:
            ...
            query = "UPDATE products SET name=%s, brand=%s, price=%s, stock=%s, image=%s, category=%s WHERE id=%s" if pg else "UPDATE products SET name=?, brand=?, price=?, stock=?, image=?, category=? WHERE id=?"
            c.execute(query, (name, brand, price, stock, image_filename, category, product_id))
            conn.commit()
            c.close()
            conn.close()
            return redirect(url_for("admin"))
